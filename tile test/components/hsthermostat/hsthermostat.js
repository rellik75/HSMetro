...
define(['jquery', 'knockout', 'devicecontroller', 'config', 'text!./hsthermostat.html', 'underscore'], function ($, ko, controller, config, templateMarkup) {
        ...
        self.toggleDevice = function (params) {
            ...
            if (_.size(self.device().controlPairs()) > 0) {
                try {
                    ...
                    self.mode(label);
                    if (self.mode() == "Cool" || self.mode()=="Cooling") {
                        self.modeIcon("img/cool.png");
                    }
                    if (self.mode() == "Heat" || self.mode()=="Heating") {
