define(['jquery', 'knockout', 'devicecontroller', 'config', 'bluebird', 'text!./hsnavidevice.html', 'underscore', 'faye'], function ($, ko, devicecontroller, config, Promise, templateMarkup) {

    function HSNaviWidget(params) {
        ...
        self.classInfo("tile-wide fg-white " + self.defaultColor);
        ...
        self.toggleDevice = function (params, event) {
            ...
            if (event.currentTarget.id=="right_nav"){
            ...
            if (event.currentTarget.id=="left_nav") {
                var o = _.find(self.device().controlPairs(), function (item) {
                    return item.Label == self.device().status();
                });
                var i = _.indexOf(self.device().controlPairs(), o);
                ...
                //debugger;
                if (i < 0) {
                    i = self.device().controlPairs().length-1;
                }
                var label = self.device().controlPairs()[i].Label;
                if (self.device().controlPairs()[i].Range != null) {
                    self.controlButtonsVisible(true);
        ...
        var setStatus = function () {
            ...
            var _classInfo="tile-wide fg-white  ";
    ...
    HSNaviWidget.prototype.dispose = function () {};


    return {
        viewModel: HSNaviWidget,
